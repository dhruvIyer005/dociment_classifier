...
try:
    from predict import DocumentClassifierPredictor  # Use your new predictor
    from config import config  # Use your new config
    from pdf_processor import PDFTextExtractor  # Use your new PDF processor
    ML_AVAILABLE = True
...
class WebDocumentPredictor:
    ...
    def load_model(self):  # <-- THIS LINE WAS NOT INDENTED PROPERLY
        """Load the trained model using your new DocumentClassifierPredictor"""
        try:
            # Check for trained model
            model_path = Path(__file__).parent.parent / "models" / "distilbert_classifier"
            ...
            if model_path.exists():
                ...
                if checkpoints:
                    ...
                    self.predictor = DocumentClassifierPredictor(str(latest_checkpoint))
                else:
                ...
                print("⚠️  Using default model as fallback")
...
if __name__ == '__main__':
    ...
    if ML_AVAILABLE:
        print(f"✓ ML modules loaded")
        print(f"✓ Target labels: {config.LABELS}")
    else:
        print("✗ ML modules not available")
    
    print(f"✓ Predictor initialized: {predictor.predictor is not None}")
    print("✓ Supported file formats: PDF, TXT")
    print("✓ Bulk processing: Enabled (up to 50 files)")
    print("="*60)

...
class DocumentClassifierPredictor:
    ...
    def _load_model(self):
        ...
        try:
            ...
            # Load label mapping - FIXED: Handle both string and integer keys
            mapping_path = self.model_dir / "label_mapping.json"
            if mapping_path.exists():
                with open(mapping_path, 'r') as f:
                    mapping = json.load(f)

                # Normalize id2label to use integer keys where possible
    ...
    def predict_single(self, text: str) -> Dict:
        ...
        try:
            ...
            # FIX: Handle both string and integer keys in id2label
            # Try to get label with integer key first, then string key
            if predicted_id in self.id2label:
                predicted_label = self.id2label[predicted_id]
            elif str(predicted_id) in self.id2label:
            ...
            # Prepare all probabilities dictionary
            all_probs = {}
